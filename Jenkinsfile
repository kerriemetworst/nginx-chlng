...
/* In order to get a date stamp define a environment variable with a script that uses the standard unix date command with 
a timecode definable as you like */
...
* to change the port that is usable I use the -p option to map the desired 8080 local port to the exposed 80 port 
...
/* if steps conclude with succes export an artifact with the archiveArtifacts command, scooping the *nginx.out made with 
the curl command */
